...
export default function ModalScanner({ onClose, onQRCodeRead = () => {}, userData }) {
  ...
  const startCamera = async () => {
    if (videoRef.current) return; // Se a câmera já estiver sendo usada, não faz nada
    try {
  ...
  const tick = () => {
    if (videoRef.current) {
      ...
      const qrCode = jsQR(imageData.data, canvas.width, canvas.height);
      if (qrCode) {
        handleScan(qrCode.data); // Tenta ler o QR Code
      }
      requestAnimationFrame(tick); // Chama a próxima iteração
    }
  };
      ...
      {paymentData ? (
        <div className={scannerStyles.paymentOverlay}>
          ...
          <div className={`${scannerStyles.paymentInfo} ${scannerStyles.card}`}>
            ...
            <p><strong>Descrição:</strong> {paymentData.description || "Pagamento via QR Code"}</p>
          ...
          <div className={scannerStyles.paymentButtons}>
            <button className={`${scannerStyles.paymentButton} ${scannerStyles.confirmButton}`} onClick={handlePayment}>
              Confirmar
            </button>
            <button className={`${scannerStyles.cancelButton} ${scannerStyles.cancelButton}`} onClick={handleCancelPayment}>
              Cancelar
            </button>
          </div>
        </div>
